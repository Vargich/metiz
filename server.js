...
async function initDatabase() {
    ...
    await run(`CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, name VARCHAR(255) NOT NULL, email VARCHAR(255) UNIQUE, phone VARCHAR(255) UNIQUE, password TEXT NOT NULL, address TEXT, is_admin INTEGER DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`);
    await run(`CREATE TABLE IF NOT EXISTS orders (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id), total NUMERIC NOT NULL, status VARCHAR(50) DEFAULT 'processing', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`);
    await run(`CREATE TABLE IF NOT EXISTS order_items (id SERIAL PRIMARY KEY, order_id INTEGER REFERENCES orders(id), product_id INTEGER REFERENCES products(id), quantity NUMERIC(10,3), price NUMERIC)`);    
    await run(`CREATE TABLE IF NOT EXISTS otp_codes (contact VARCHAR(255) PRIMARY KEY, code VARCHAR(50), expires_at BIGINT)`);
    await run(`CREATE TABLE IF NOT EXISTS tg_users (phone VARCHAR(255) PRIMARY KEY, chat_id VARCHAR(255))`);

    // Миграция колонки скидки
    await run(`ALTER TABLE users ADD COLUMN IF NOT EXISTS discount INTEGER DEFAULT 0`);
...
const authenticateToken = async (req, res, next) => {
    ...
    try {
        ...
        const user = await queryOne("SELECT id, name, email, phone, is_admin, discount FROM users WHERE id = ?", [decoded.userId]);
...
app.get('/api/users', authenticateToken, isAdminMiddleware, async (req, res) => {
    try {
        const users = await queryAll(
            "SELECT id, name, email, phone, is_admin, discount, created_at FROM users ORDER BY id ASC"
        );
        res.json(users);
    } catch (err) {
...
app.get('/api/auth/me', authenticateToken, (req, res) => {
    res.json({ user: { id: req.user.id, email: req.user.email, phone: req.user.phone, name: req.user.name, displayName: req.user.name, isAdmin: !!req.user.is_admin, discount: req.user.discount || 0 } });
});
...
app.get('/api/orders', authenticateToken, async (req, res) => {
    if (req.user.is_admin && req.query.all === 'true') {
        res.json(await queryAll("SELECT o.*, u.name as user_name, u.email as user_email, u.phone as user_phone FROM orders o LEFT JOIN users u ON o.user_id = u.id ORDER BY o.created_at DESC"));
    } else {
...
// Безопасное изменение состава заказа с учетом CAST в SQL
app.put('/api/orders/:id/items', authenticateToken, isAdminMiddleware, async (req, res) => {
    ...
    try {
        ...
        for (const item of items) {
            ...
            if (!isCancelled && Number(product.quantity) < qty) {
                throw new Error(`Недостаточно товара "${product.name}" на складе. Доступно: ${product.quantity}`);
            }

            if (!isCancelled) {
                await client.query(convertPgQuery("UPDATE products SET quantity = quantity - CAST(? AS NUMERIC) WHERE id = ?"), [qty, prodId]);
            }
        ...
        console.error("Ошибка при изменении состава заказа:", err);
...
app.post('/api/orders', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const orderId = await run("INSERT INTO orders (user_id, total) VALUES (?, ?)", [req.user.id, serverTotal]);
        for (const item of validItems) {
            const originalPrice = Number(item.price);
            const discountedPrice = discountPercent > 0 ? (originalPrice * (1 - discountPercent / 100)) : originalPrice;
            await run("INSERT INTO order_items (order_id, product_id, quantity, price) VALUES (?, ?, ?, ?)", [orderId, item.id, item.quantity, discountedPrice]);
            await run("UPDATE products SET quantity = quantity - ? WHERE id = ?", [item.quantity, item.id]);
        }
